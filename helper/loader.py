...
def build_image_set(file_path,data_dir_in, channels=1, scale=1, convert_ycbcr=True, resampling_method="bicubic",
                    print_console=True):
    ...
    path_out=data_dir_in+'/'+name
    input_image2 = util.load_image(path_out, print_console=False)
    input_image = util.convert_rgb_to_y(input_image2)
...
class BatchDataSets:
    ...
    def build_batch(self, data_dir, data_dir_in):
        ...
        for filename in filenames:
            ...
            input_image, input_interpolated_image, true_image = \
                build_image_set(filename,data_dir_in, channels=self.channels, resampling_method=self.resampling_method,
    ...
    def load_batch_image(self, max_value):
        ...
        if max_value == 255:
            ...
            #print(np.max(np.multiply(self.input_images[number], 4.)))
