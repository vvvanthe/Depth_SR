...
def random_crop(lr_img, hr_img,bi_img, hr_crop_size=96, scale=2):
    lr_crop_size = hr_crop_size // scale
    lr_img_shape = tf.shape(lr_img)[:2]

    lr_w = np.random.randint(low=0,high=lr_img_shape[0] - lr_crop_size + 1)
    lr_h = np.random.randint(low=0,high=lr_img_shape[0] - lr_crop_size + 1)
    ...
    bi_img_cropped = bi_img[bi_h:bi_h + hr_crop_size, bi_w:bi_w + hr_crop_size]
    return lr_img_cropped, hr_img_cropped,bi_img_cropped

def random_rotate(lr_img, hr_img, bi_img):
    rn = np.random.randint(low=0,high=4)
    return np.rot90(lr_img, rn), np.rot90(hr_img, rn), np.rot90(bi_img, rn)

def random_patch(lr_img, hr_img,bi_img, hr_crop_size=96, scale=2):
    lr_img_cropped, hr_img_cropped, bi_img_cropped=random_crop(lr_img, hr_img,bi_img, hr_crop_size=hr_crop_size, scale=scale)
    return random_rotate(lr_img_cropped, hr_img_cropped, bi_img_cropped)
