...
class SuperResolution(tf_graph.TensorflowGraph):
    ...
    def vgg_loss(self,y_true, y_pred):
        ...
        y_pred3 = tf.keras.layers.concatenate([y_pred, y_pred, y_pred])  # tf.repeat(y_pred,repeats=3,axis=-1)
        y_true3 = tf.keras.layers.concatenate([y_true, y_true, y_true])  # tf.repeat(y_true,repeats=3,axis=-1)
    ...
    def load_datasets(self, data_dir, batch_dir, batch_image_size, data_dir_in, stride_size=0):
        ...
        if not self.train.is_batch_exist():
            self.train.build_batch(data_dir,data_dir_in)
        else:
    ...
    def build_graph(self):
        ...
        num_feat_in = np.array([32, 32, 16, 16, 16, 8, 8])
        #num_feat_in = np.array(([192,160,160,128,128,96,96,96,64,64,32,32]))
        ...
        for i in range(self.layers):
            if self.min_filters != 0 :#and i > 0:
                ...
                output_feature_num= num_feat_in[i]
        ...
        input_channels=total_output_feature_num # new line
        if self.pixel_shuffler:
            ...
            if self.scale == 4:
                ...
                self.build_pixel_shuffler_layer("Up-PS",self.H_concat , self.scale,#self.H[-1], self.scale,
                                                input_channels, output_channels,
                                                depthwise_separable=self.depthwise_separable)
    ...
    def build_optimizer(self):
        ...
        # if using vgg loss
        self.mse = tf.reduce_mean(tf.square(diff, name="diff_square"), name="mse")
        self.image_loss = tf.identity(self.mse, name="image_loss")#self.vgg_loss(self.y_, self.y)*1e7
    ...
    def train_batch(self):
        ...
        _, image_loss, mse, test = self.sess.run([self.training_optimizer, self.image_loss, self.mse, self.y_], feed_dict=feed_dict)
    ...
    def print_status(self, psnr, ssim, log=False):
        ...
        if self.step == 0:
            ...
            print("Loss value: percep {}  and mse: {}".format(self.training_loss_sum / self.training_step,
                                                              self.training_mse_sum / self.training_step))
    ...
    def do_for_file(self, file_path, output_folder="output"):
        ...
        if len(org_image.shape) >= 3 and org_image.shape[2] == 3 and self.channels == 1:
            ...
            image =  np.clip(util.convert_y_and_cbcr_to_rgb(output_y_image, scaled_ycbcr_image[:, :, 1:3]),0, 255)
        else:
            scaled_image = util.resize_image_by_pil(org_image, self.scale, resampling_method=self.resampling_method)
            #util.save_image(output_folder + filename + "_bicubic_y" + extension, scaled_image)
            image = np.clip(self.do(org_image),0, 255)

        bic_image=util.resize_image_by_pil(org_image,scale=2)
        util.save_image(output_folder +"/bic/" + filename + extension, bic_image)
        util.save_image(output_folder + filename  + extension, image)

    def generate_HR_frame(self,frame,i):
        org_image =frame
        scaled_image = util.resize_image_by_pil(org_image, self.scale, resampling_method=self.resampling_method)
        if len(scaled_image.shape) >= 3 and scaled_image.shape[2] == 3 and self.channels == 1:
            input_y_image = util.convert_rgb_to_y(org_image)
            output_y_image = self.do(input_y_image)
            scaled_ycbcr_image = util.convert_rgb_to_ycbcr(scaled_image)
            image = np.clip(util.convert_y_and_cbcr_to_rgb(output_y_image, scaled_ycbcr_image[:, :, 1:3]),0, 255)
        else:
            image = np.clip(self.do(org_image),0, 255)
        #util.save_image("./Video/iamgesr/frame"+str(i)+".png", image)
    ...
    def do_for_evaluate_with_output(self, file_path,output_directory, print_console=False):
        ...
        util.save_image(output_directory+'/bic/' + filename + extension, input_bicubic_image)
        ...
        if true_image.shape[2] == 3 and self.channels == 1:
            ...
            output_color_image = np.clip(util.convert_y_and_cbcr_to_rgb(output_y_image, true_ycbcr_image[:, :, 1:3]),0, 255)
            ...
            util.save_image(output_directory +'color/'+ filename + extension, output_color_image)
